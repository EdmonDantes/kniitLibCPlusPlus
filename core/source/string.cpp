...
KNIIT_LIB_NAMESPACE {
    ...
    String::String(List<uint8_t>& bytes, const Codec *codec, ByteOrder byteOrder) {
        if (this->value != nullptr) {
            delete[] this->value;
        }

        InputListStream<uint8_t> in(&bytes, false);
        this->value = new List(std::move(codec->decode(&in, byteOrder)));
    }
    ...
    List<uint8_t> String::to(const Codec *codec, bool addZeroToEnd, ByteOrder byteOrder) const {
