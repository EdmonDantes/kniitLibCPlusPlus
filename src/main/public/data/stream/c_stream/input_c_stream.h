...
KNIIT_LIB_NAMESPACE {
    ...
    KNIIT_LIB_CLASS InputCStream : public InputStream<uint8> {
    protected:
    ...
    public:
        ...
        uint8 read();
        uint8 read(uintmax position);
        void read(uint8* pointer, uintmax length);
        void read(uint8* pointer, uintmax length, uintmax position);
    };
