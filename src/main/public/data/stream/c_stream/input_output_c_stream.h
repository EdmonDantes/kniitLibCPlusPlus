...
KNIIT_LIB_NAMESPACE {
    ...
    KNIIT_LIB_CLASS InputOutputCStream : public InputOutputStream<uint8> {
     private:
     ...
     public:
        ...
        uint8 read() override;
        uint8 read(uintmax position) override;
        void read(uint8* pointer, uintmax length) override;
        void read(uint8* pointer, uintmax length, uintmax position) override;

        bool canWrite() const override;
        bool flush() override;
        bool write(uint8& obj) override;
        bool write(uint8& obj, uintmax position) override;
        bool write(uint8* obj, uintmax length) override;
        bool write(uint8* obj, uintmax length, uintmax position) override;
